...
You are teamwork_preview_reviewer_m2_2. Your working directory is: /Volumes/Storage/bigworkspace/spectral-pruner/.agents/teamwork_preview_reviewer_m2_2
Read:
...
1. Verify mathematical correctness of Arrington Clamping, shifted Laplacian SpMV, and Rayleigh quotient connectivity.
2. Verify that `prune` and `prune_with_workspace` yield 100% identical numerical results on all topologies.
3. Run `cargo check --all-targets`, `cargo test --all-targets`, `cargo clippy --all-targets -- -D warnings`, `cargo tree`.
4. Produce a structured review in /Volumes/Storage/bigworkspace/spectral-pruner/.agents/teamwork_preview_reviewer_m2_2/handoff.md with an explicit verdict: APPROVE or REQUEST_CHANGES.
